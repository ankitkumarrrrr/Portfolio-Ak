...
const projectData = {
  ...
  'hyperlocal-hand': {
    ...
    tagline: 'Buy back your time — verified taskers nearby, live GPS tracking, and escrow-backed payments for neighborhood errands.',
    thumbnail: '/projects/hyperlocal-thumb.jpg',
    liveUrl: '#',
    githubUrl: 'https://github.com/ankitkumarrrrr/hyperlocal-hand-app',
    tech: ['JavaScript', 'Live GPS Tracking', 'Real-Time Matching', 'Escrow Payments', 'REST APIs'],
    role: 'Full-Stack Developer',
    duration: 'Ongoing',
    problem: 'Everyday errands eat the one resource you cannot earn back: time. Formal gig platforms are built for long-haul deliveries, not the five-minute task down the street — and informal help has no verification, no price transparency, and no payment protection.',
    solution: 'Built a hyperlocal marketplace that turns neighborhood errands into a micro-economy: tasks are matched instantly to verified taskers within walking distance, both parties follow the errand on a live GPS map, and payments sit in escrow until the task is confirmed complete — protecting the requester and the tasker equally.',
    approach: [
      'Instant local matching: rank nearby verified taskers by distance, rating, and current load',
      'Live GPS tracking of the assigned tasker from acceptance to completion, streamed to both clients',
      'Escrow-backed payment flow: funds locked at booking, released only on tasker confirmation',
      'Trust layer: tasker verification and rating history surfaced before every booking',
      'Transparent pricing so errands settle fast and fairly instead of haggling',
    ],
    results: [
      { metric: 'GPS', label: 'Live tracking from task acceptance to completion' },
      { metric: 'Escrow', label: 'Payments held until the job is confirmed done' },
      { metric: '2-sided', label: 'Ratings and verification for requesters and taskers' },
      { metric: '<5min', label: 'Target from posting to verified tasker matched' },
    ],
    challenges: [
      'Location drift on mobile GPS → smoothed position updates and snapped paths to streets',
      'Trust between strangers → verification plus escrow removes the incentive to defect',
      'Matching fairness → weighted distance against rating and workload so the same tasker is not starved or overloaded',
    ],
    learnings: 'Marketplace trust is a systems problem, not a UI badge. Escrow plus verification plus live tracking each close a different loophole — you need all three before strangers transact happily.',
    images: [],
  },
  learnreels: {
    title: 'LearnReels',
    subtitle: 'Real-Time Learning Backend',
    tagline: 'The engine room: Socket.io live chat and notifications, Cloudinary media pipeline, and the API surface the whole app runs on.',
    thumbnail: '/projects/learnreels-thumb.jpg',
    liveUrl: '#',
    githubUrl: 'https://github.com/ankitkumarrrrr/learnreels_backend',
    tech: ['Node.js', 'Socket.io', 'Cloudinary', 'Express', 'REST APIs', 'Real-Time Systems'],
    role: 'Backend Engineer',
    duration: 'Ongoing',
    problem: 'Learning apps live or die on engagement, and engagement is real-time: a doubt asked during a reel needs an answer while the reel is still playing. Polling-based backends add seconds of lag, and media handling at scale quickly bloats app servers.',
    solution: 'Built the real-time backend for LearnReels: Socket.io channels for instant chat and notifications that land the moment a reel is watched, a Cloudinary pipeline that ingests, transforms, and serves all media off-app, and a clean REST API surface the mobile client consumes for everything else.',
    approach: [
      'Socket.io event architecture for live chat rooms and push notifications — no polling, no lag',
      'Cloudinary integration for media upload, on-the-fly transformation, and CDN-backed delivery',
      'REST API layer decoupled from sockets so clients resync cleanly after disconnects',
      'Connection lifecycle handling: reconnects, presence, and missed-message recovery',
    ],
    results: [
      { metric: 'Live', label: 'Socket.io chat + notifications with no polling lag' },
      { metric: 'CDN', label: 'Cloudinary pipeline offloads all media traffic' },
      { metric: 'API', label: 'Clean REST surface decoupled from realtime layer' },
      { metric: '0', label: 'Media bytes touching the app server' },
    ],
    challenges: [
      'Socket lifecycle edge cases → explicit reconnect + missed-event recovery so chat never silently drops',
      'Upload reliability on flaky networks → resumable uploads and server-side validation before CDN publish',
      'Keeping sockets and REST state consistent → single source of truth with events fanned out to both',
    ],
    learnings: 'Real-time is an architecture decision, not a feature flag. The moment chat and notifications became event-driven, the product felt alive — and moving media off the app server made scaling a non-problem before it ever was one.',
    images: [],
  },
  gesturesense: {
    title: 'GestureSense',
